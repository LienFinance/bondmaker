...
export const DECIMALS_OF_STRIKE_PRICE = 8;
...
export const vmExceptionTemplate = (errorMessage: string) =>
  `Returned error: VM Exception while processing transaction: revert ${errorMessage} -- Reason given: ${errorMessage}.`;
...
export function lineSegmentToFnMap(
  ...
  const data = polyline.map((segment) => {
    ...
    const payload =
      web3.eth.abi
        .encodeParameter("uint64", ten.pow(decimal).times(x1).dp(0).toString())
        .slice(-16) +
      web3.eth.abi
        .encodeParameter("uint64", ten.pow(decimal).times(y1).dp(0).toString())
        .slice(-16) +
      web3.eth.abi
        .encodeParameter("uint64", ten.pow(decimal).times(x2).dp(0).toString())
        .slice(-16) +
      web3.eth.abi
        .encodeParameter("uint64", ten.pow(decimal).times(y2).dp(0).toString())
        .slice(-16);
...
export function fnMapToLineSegment(fnMap: string, decimal: number = 8) {
  const ten = new BigNumber(10);
...
export function calcFnMap(
  solidStrikePrice: BigNumber.Value,
  decimal: number = 8
) {
  const ssp = new BigNumber(solidStrikePrice).dp(decimal);
  ...
  const fnMaps = [
    {fnMap: lineSegmentToFnMap(fnMapSolid, decimal)},
    {fnMap: lineSegmentToFnMap(fnMapLiquid, decimal)},
  ];

  return fnMaps;
}

export function getSolidStrikePrice(fnMap: string): BigNumber {
  ...
  const solidStrikePrice = new BigNumber(polyline[0][2]);
  for (let i = 0; i < polyline.length; i++) {
    if (!solidStrikePrice.eq(polyline[i][3])) {
      return new BigNumber(0);
    }
  }

  return solidStrikePrice;
}

export function getRateLBTWorthless(fnMap: string): BigNumber {
  const polyline = fnMapToLineSegment(fnMap);
  if (!(polyline[0][0] === 0 && polyline[0][1] === 0)) {
    return new BigNumber(0);
  }
  ...
  const rateLBTWorthless = new BigNumber(polyline[0][2]);
  for (let i = 0; i < polyline.length; i++) {
    if (!rateLBTWorthless.plus(polyline[i][3]).eq(polyline[i][2])) {
      return new BigNumber(0);
    }
  }
...
export const fromE0Amount = (amount: BigNumber.Value, decimals: number) =>
  new BigNumber(amount.toString()).shiftedBy(decimals).dp(0).toString(10);

export const toE0Amount = (
  amount: BigNumber.Value | BN,
  decimals: number = DECIMALS_OF_BOND_AMOUNT
...
export const fromBTAmount = (
  amount: BigNumber.Value,
  decimals: number = DECIMALS_OF_BOND_AMOUNT
) => new BigNumber(amount.toString()).shiftedBy(decimals).dp(0).toString(10);

export const toBTAmount = (
  amount: BigNumber.Value | BN,
  decimals: number = DECIMALS_OF_BOND_AMOUNT
) => new BigNumber(amount.toString()).shiftedBy(-decimals);
...
export const getEthBalance = async (ownerAddress: string) => {
  const ethBalance = await web3.eth.getBalance(ownerAddress);
  return toEtherAmount(ethBalance);
};

/**
 * ```
 * assert(beEqualToWithoutFraction( 1.13,  1.1, undefined, BigNumber.ROUND_DOWN) === true);  // 1.1  === 1.1
 * assert(beEqualToWithoutFraction( 1.09,  1.1, undefined, BigNumber.ROUND_DOWN) === false); // 1.0  === 1.1
 * assert(beEqualToWithoutFraction( 1.09,  1.1, undefined,            undefined) === true);  // 1.1  === 1.1
 * assert(beEqualToWithoutFraction( 1.99,    1, undefined, BigNumber.ROUND_DOWN) === true);  // 1.   === 1.
 * assert(beEqualToWithoutFraction( 1.99,    1,         1, BigNumber.ROUND_DOWN) === false); // 1.9  === 1.0
 * ```
 *
 * @param actual
 * @param expected
 ...
 * @param roundingMode
 */
export const beEqualToWithoutFraction = (
  actual: BigNumber.Value,
  expected: BigNumber.Value,
  decimalPlace?: number,
  roundingMode?: 0 | 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8
) => {
  let actualBN = new BigNumber(actual);
  let expectedBN = new BigNumber(expected);
  if (decimalPlace !== undefined) {
    actualBN = actualBN.dp(decimalPlace, roundingMode);
    expectedBN = expectedBN.dp(decimalPlace, roundingMode);
  } else if (actualBN.dp() < expectedBN.dp()) {
    expectedBN = expectedBN.dp(actualBN.dp(), roundingMode);
  } else {
    actualBN = actualBN.dp(expectedBN.dp(), roundingMode);
  }

  return actualBN.eq(expectedBN);
};

export const beNearlyEqualTo = (
  actual: BigNumber.Value,
  expected: BigNumber.Value,
  accuracy: number = 1
) => {
  ...
  // If the sign of expected is the same as that of actual.
  if (expectedBN.times(actual).lt(0)) {
    return false;
  }

  // | 1 - actual / expected | <= 1 - accuracy
  return expectedBN
    .minus(actual)
    .abs()
    .lte(expectedBN.abs().times(1 - accuracy));
};

export const omitUndefinedValue = <T extends {}>(obj: T): T => {
...
export function applyDecimalGap(
  baseAmount: number,
  decimalsOfBase: number,
  decimalsOfQuote: number
): number;
export function applyDecimalGap(
  baseAmount: string,
  decimalsOfBase: number,
...
export function applyDecimalGap(
  baseAmount: number | string | BigNumber,
  decimalsOfBase: number,
  decimalsOfQuote: number
): number | string | BigNumber {
  if (typeof baseAmount === "number") {
    if (decimalsOfQuote > decimalsOfBase) {
      return Math.floor(baseAmount * 10 ** (decimalsOfQuote - decimalsOfBase));
    }

    return Math.floor(baseAmount / 10 ** (decimalsOfBase - decimalsOfQuote));
  } else if (typeof baseAmount === "string") {
    return new BigNumber(baseAmount)
      .shiftedBy(decimalsOfQuote - decimalsOfBase)
